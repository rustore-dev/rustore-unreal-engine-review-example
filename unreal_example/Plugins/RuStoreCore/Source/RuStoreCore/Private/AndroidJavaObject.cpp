...
#if PLATFORM_ANDROID
AndroidJavaObject::AndroidJavaObject(JNIEnv* env, jclass javaClass, jobject javaObject, const FString& className, bool bIsGlobalRef, bool bOwnsJavaClass)
{
	this->env = env;
	this->javaClass = javaClass;
	this->javaObject = javaObject;
	this->className = className;
	this->bIsGlobalRef = bIsGlobalRef;
	this->bOwnsJavaClass = bOwnsJavaClass;
}
#else
AndroidJavaObject::AndroidJavaObject(const FString& className)
{
	this->className = className;
}
#endif
...
{
#if PLATFORM_ANDROID
	if (javaObject != nullptr && !env->IsSameObject(javaObject, nullptr))
	{
		if (bIsGlobalRef) env->DeleteGlobalRef(javaObject);
	}
	if (javaClass != nullptr && bOwnsJavaClass)
	{
...
AndroidJavaObject* AndroidJavaObject::GetStaticAJObject(FString fieldName, FString className)
{
	...
	if (localRef != nullptr)
	{
		...
		result = new AndroidJavaObject(env, globalClass, globalRef, "", true, true);
	}
#endif
...
AndroidJavaObject* AndroidJavaObject::GetAJObjectArrayElement(int i)
{
	...
	if (element != nullptr)
	{
		jclass localClass = env->GetObjectClass(element);
		jclass globalClass = (jclass)env->NewGlobalRef(localClass);
		env->DeleteLocalRef(localClass);
		jobject globalRef = env->NewGlobalRef(element);
